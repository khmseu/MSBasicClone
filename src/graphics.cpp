...
#include <cstdlib>
  ...
  // On Windows: include Windows headers only when Raylib is not in use.
  // If Raylib is enabled we avoid pulling Windows-only headers to prevent
  // symbol collisions and to keep the build portable across platforms.
  ...
  #endif
#else
  // Non-Windows platforms need POSIX headers
  #include <sys/ioctl.h>
  #include <unistd.h>
#endif
...
void Graphics::renderFrame() {
  ...
  for (const auto &sample : frame_) {
    renderer_->drawPixel(static_cast<int>(sample.logicalX),
                         static_cast<int>(sample.logicalY), sample.color);
  }
 ...
 *
 * Sets the current position for shape drawing without actually plotting a
 * point. Used primarily with DRAW/XDRAW to set the origin for shape operations.
 *
...
/**
 * @brief Query the current terminal size (columns/rows)
 *
 * Determines terminal dimensions for text-mode rendering decisions, using
 * platform-specific methods where available. On POSIX systems this uses
 * ioctl(TIOCGWINSZ); on Windows the Win32 console API is used. When
 * those techniques are unavailable the function may fall back to the
 * COLUMNS/LINES environment variables. The function returns true when a
 * valid size was determined and populates the output parameters.
 *
 * @param columns Output: number of columns (characters)
