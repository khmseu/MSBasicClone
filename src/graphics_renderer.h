...
// Raylib types are needed by-value when HAVE_RAYLIB is enabled (Font member).
// When graphics are disabled or Raylib isn't available, we avoid pulling in
// the dependency entirely.
#ifdef HAVE_RAYLIB
#include <raylib.h>
#else
// Forward declarations for Raylib-like types (for stub builds).
struct Color;
struct Font;
#endif
