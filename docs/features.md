...
- Built-in functions: SIN, COS, TAN, ATN, EXP, LOG, SQR, ABS, INT, SGN, RND, LEN, VAL, ASC, CHR$, LEFT$, RIGHT$, MID$, STR$, PEEK(), SCRN(x,y) pixel query, USR(addr) (stub returns 0).
...
- Graphics: GR/HIRES configure a scaled overlay window sized to the terminal and track scaled plot samples (with color). Shape table support with default shapes (triangle, square) and user-defined shapes via SHLOAD; DRAW/XDRAW apply rotation (ROTATE) and scale (SCALE) transforms and record transformed vertices using optional AT-origin or last plot position; XDRAW implements XOR behavior toggling pixels on repeated draws. SHLOAD loads user-defined shapes from DATA statements—format: shape number (int), point count (int), followed by x,y coordinate pairs. SCRN(x,y) queries pixel buffer and returns color. Still no on-screen rendering.
...
- On-screen graphics: Actual rendering of plotting primitives and shape tables.
- Shape tables: Binary shape table loading/storing (current implementation uses DATA-driven text format via SHLOAD).
- Memory/state queries: FRE remains a fixed placeholder; PDL returns 0; HIMEM/LOMEM boundaries are tracked but not enforced.
- Extended I/O: Additional file/device I/O beyond LOAD/SAVE/CATALOG.
- WAIT behavior: WAIT addr,mask polls indefinitely; real Applesoft supports timeout/external events.
