...
void Interpreter::parseLine(const std::string& line, LineNumber& lineNum, std::string& code) {
    ...
    if (pos < line.length() && std::isdigit(line[pos])) {
        // Extract line number
        size_t numEnd = pos;
        while (numEnd < line.length() && std::isdigit(line[numEnd])) numEnd++;
        
        lineNum = std::stoi(line.substr(pos, numEnd - pos));
        
        // Rest is code
        while (numEnd < line.length() && std::isspace(line[numEnd])) numEnd++;
...
void Interpreter::deleteLine(LineNumber lineNum) {
    program_.erase(lineNum);
}
...
bool Interpreter::isInForLoop(const std::string& varName) {
    for (const auto& loop : forStack_) {
        if (loop.varName == varName) return true;
    }
    return false;
}
