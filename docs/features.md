...
- Statements: PRINT/? with separators; INPUT with optional prompt; LET/assign; IF/THEN[/ELSE] inline or to line numbers; GOTO; GOSUB/RETURN; FOR/NEXT with STEP; WHILE/WEND for conditional loops; CLR to reset variables/loops; END; REM comment; COLON separators; TRACE/NOTRACE to echo executed line numbers; RANDOMIZE seed to initialize RND; SPEED n (clamped 0–255 ms) to insert per-statement delay; PR#n/IN#n stubs store device slots while keeping screen/keyboard active; POP to remove entry from GOSUB stack; WAIT addr,mask to poll I/O; HIMEM=/LOMEM= to set memory boundaries; HPLOT for high-resolution graphics plotting (points and lines); MOVE x,y for cursor positioning in hi-res graphics; ROTATE angle and SCALE n for shape parameters; DRAW/XDRAW with optional AT x,y integrated with graphics shape tables and transforms (see Graphics stubs below).
...
- Graphics stubs: GR/HIRES configure a scaled overlay window sized to the terminal and track scaled plot samples (with color). Basic shape table support: default shapes (triangle, square) available; DRAW/XDRAW apply rotation (ROTATE) and scale (SCALE) and record transformed vertices with optional AT-origin or last plot position. Still no on-screen rendering.
...
- On-screen graphics: Actual rendering of plotting primitives and shape tables; XOR behavior for XDRAW; SCRN(x,y) should reflect display state (currently returns 0).
- Shape tables: User-defined shape table loading/storing and full management beyond defaults; ensure ROTATE/SCALE apply during rendering.
